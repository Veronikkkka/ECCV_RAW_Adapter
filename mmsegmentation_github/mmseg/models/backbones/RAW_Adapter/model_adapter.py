...
# Model-level Adapter Generation 
...
if __name__ == "__main__":
    ...
    model_adapter = Model_level_Adapeter(in_c=3, in_dim=16, w_lut=True)
    print(sum(param.numel() for param in model_adapter.parameters()))
    IMGS = [torch.randn(2,3,512,512), torch.randn(2,3,512,512), torch.randn(2,3,512,512), torch.randn(2,3,512,512)]
    adapter = model_adapter(IMGS)
    

    fea_1 = torch.randn(2, 64, 128, 128)
    fea_2 = torch.randn(2, 128, 64, 64)
    fea_3 = torch.randn(2, 320, 32, 32)
    
    merge_1 = Merge_block(fea_c=64, ada_c=16, mid_c=64, return_ada=True)
    merge_2 = Merge_block(fea_c=128, ada_c=32, mid_c=96, return_ada=True)
    merge_3 = Merge_block(fea_c=320, ada_c=64, mid_c=128, return_ada=False)
